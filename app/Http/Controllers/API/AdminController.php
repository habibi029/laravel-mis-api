...
class AdminController extends Controller
{
    ...
    public function markAbsent(Request $request) 
    {
        ...
        $attendance = $this->createOrUpdateAttendance($request->staff_id, 'Absent');
    ...
    public function markLeave(Request $request)
    {
        ...
        $attendance = $this->createOrUpdateAttendance($request->staff_id, 'Leave');
    ...
    public function employee_clockIn(Request $request)
{
    ...
    if ($request->date !== $today) {
        return response()->json(['message' => 'You can only clock in/out for today.'], 400);
    }
    ...
    if (!$attendance->exists) {
        // Set default values if the attendance record is new
        $attendance->fullname = $request->fullname;
        $attendance->attendance_status = 'Absent';
    }

    if ($request->clock_type === 'clock_in_time') {
        return $this->handleClockIn($attendance, $request->time);
    }
    ...
    return response()->json(['message' => 'Invalid clock type.'], 400);
}
...
private function handleClockIn($attendance, $time)
{
...
//endcode
    ...
    {
        if ($id) {
            // If an ID is provided, fetch the attendance of the specific user
            $attendances = EmployeeAttendance::where('staff_id', $id)->get();
        } else {
            // If no ID is provided, fetch all attendances
    ...
    public function store_staff_payrolls(Request $request, $id)
    {
        ...
        // Deductions using custom rates from request
        $sss_rate = $request->sss_rate / 100;
        $pagibig_rate = $request->pagibig_rate / 100;
        $philhealth_rate = $request->philhealth_rate / 100;
        ...
        // Store the calculated payroll for the staff member
        $payroll = new Payroll();
        ...
        $payroll->total_salary = $total_salary;   // Store total salary
        $payroll->overtime = $overtime;           // Store overtime
        $payroll->yearly_bonus = $yearly_bonus;   // Store yearly bonus
        $payroll->sales_comission = $sales_comission; // Store sales commission
        $payroll->incentives = $incentives;       // Store incentives
        $payroll->sss_deduction = $sss;           // Store SSS deduction
        $payroll->pagibig_deduction = $pag_ibig;  // Store Pag-IBIG deduction
        $payroll->philhealth_deduction = $philhealth; // Store PhilHealth deduction
        $payroll->final_salary = $final_salary;   // Store final salary after deductions
        ...
        return response()->json([
            ...
            'data' => $payroll->load('staff') // ito ang magic
        ], 200);
    }
